...
public class just_numbering_DFS {
	...
	static int DFS(int y , int x) {
		...
		while(!queue.isEmpty()) {
			...
			for(int i = 0 ; i<4 ; i++) {
				...
				if(0<=dexY && dexY<n && 0<=dexX && dexX<n) {
					if(map[dexY][dexX] == 1 && !check[dexY][dexX]) {
						check[dexY][dexX] = true;
						map[dexY][dexX] = 0;
						queue.add(new member(dexY,dexX));
						cnt++;
					}
